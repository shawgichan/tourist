...
func (server *Server) LoginUser(ctx context.Context, req *pb.LoginUserRequest) (*pb.LoginUserResponse, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, status.Errorf(codes.NotFound, "username not found %w", err)
		} else {
			return nil, status.Errorf(codes.Internal, "error while getting user %w", err)
		}
	}

	passwordError := utils.CheckPassword(req.Password, user.HashedPassword)
	if passwordError != nil {
		return nil, status.Errorf(codes.PermissionDenied, "wrong password %w", err)
	}

	accessToken, err2 := server.TokenMaker.CreateToken(user.Username, 2*time.Hour)
	if err2 != nil {
		return nil, status.Errorf(codes.Internal, "error generating token %w", err2)
	}
